...
export async function storeNominationImage(context: AppLoadContext, actor: CurrentUser, nominationId: string, file: File, kind: "tweet_avatar" | "nomination_image") {
  ...
  const storage = new R2ObjectStorage(env.MEDIA_BUCKET, env.APP_BASE_URL);
  const extension = file.type.split("/")[1]?.replace("jpeg", "jpg") ?? "bin";
  const key = `${actor.id}/${nominationId}/${kind}-${newId("obj")}.${extension}`;
  const stored = await storage.put({ key, body: await file.arrayBuffer(), contentType: file.type });
